...
export default class Service {
  ...
  #checkEar(left, right){
    if(right && !left) console.log("right ear");
    if(!right && left) console.log("left ear");
    return "";
  };
  ...
  async handBlinked(video){
    ...
    for(const prediction of predictions){
      ...
      if(leftEAR <= EAR_THRESHOLD && rightEAR <= EAR_THRESHOLD){
        return blinked;
      }else if(this.#checkEar(leftEAR <= EAR_THRESHOLD, rightEAR <= EAR_THRESHOLD)) return; 
      continue;
    };
   return false;
  };
